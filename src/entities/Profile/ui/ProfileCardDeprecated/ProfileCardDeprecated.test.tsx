...
describe('ProfileCardDeprecated', () => {
  test('Рендер в режиме чтения', () => {
    componentRender(<ProfileCardDeprecated {...defaultProps} />);
  ...
  test('Рендер без аватара', () => {
    componentRender(
      <ProfileCardDeprecated
        {...defaultProps}
        data={{ ...mockProfile, avatar: undefined }}
      />,
    );

    // Проверяем, что аватар не отображается
    const avatar = screen.queryByAltText('Profile avatar');
